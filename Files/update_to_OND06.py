...
def update_GA_to_OND06(data_pkg_dir, subplatform_code='GNAC'):
    ...
    def update_filename(filename):
        ...
        location = location_mapping[s_filename[-1]] if s_filename[-1] in location_mapping else s_filename[-1] + '.%s' % ext
        new_filename = '_'.join(subject + [subplatform_code, device, location]) + '.%s' % ext
        return new_filename
    ...
    for f in csv_files:
        ...
        f = f.replace('Accelerometer', 'ACCELEROMETER'
